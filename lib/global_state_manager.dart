...
class GlobalStateManager {
  ...
  Future<Roles> getRole(String walletAddress) async {
    ...
    bool isGuardian = await _currentSocialRecoveryWallet.isGuardian(Uint8List.fromList(hex.decode(_keccak256(walletAddress).substring(2))));
  ...
  Future<String> submitTransaction(String destinationAddress, double amount) async {
    String txHash = await _currentSocialRecoveryWallet.submitTransaction(EthereumAddress.fromHex(destinationAddress), BigInt.from(amount * pow(10, 18)), Uint8List.fromList(hex.decode("0x".substring(2))), credentials: _credentials);
    return txHash;
  }

  Future<String> executeTransaction(int txIndex) async {
    String txHash = await _currentSocialRecoveryWallet.executeTransaction(BigInt.from(txIndex), credentials: _credentials);
    return txHash;
  }

  Future<String> confirmTransaction(int txIndex) async {
    String txHash = await _currentSocialRecoveryWallet.confirmTransaction(BigInt.from(txIndex), credentials: _credentials);
    return txHash;
  }

  Future<String> confirmChangeSpenderRequest(int reqIndex) async {
    String txHash = await _currentSocialRecoveryWallet.confirmChangeSpenderRequest(BigInt.from(reqIndex), credentials: _credentials);
    return txHash;
  }

  Future<String> submitChangeSpenderRequest(String newSpender) async {
    String txHash = await _currentSocialRecoveryWallet.submitChangeSpenderRequest(EthereumAddress.fromHex(newSpender), credentials: _credentials);
    return txHash;
  }
